#!/usr/bin/env bash
  ...
  local dest_dir='.'
  which xz &> /dev/null  || echo No xz present.  Please install xz.
  which xz &> /dev/null  || exit
  time for u in "$@"
   do
    case ${u} in
      -h|--help)
       echo "xzarchive [--help] [ -d=path/to/archive/directory ] {directory_root}"
     ;;
      -d=*)
       dest_dir="${u#*=}"
     ;;
      *)
       ...
       tar cJf ${dest_dir}/__${u}__ ${u}/
       echo "Hashing archive for __${u}__ [$(date)]"
       mv -v ${dest_dir}/__${u}__  ${dest_dir}/${u}_$(date "+%Y-%m-%d")_$(sha1sum -b ${dest_dir}/__${u}__ | awk '{ print $1}').tar.xz
     ;;
    esac
   done
